...
def detail(request, bid):
    try:
        ...
        book = get_object_or_404(BookInfo, pk=bid)
...
def addhandle(request):
    ...
    return redirect(reverse('booktest:booklist'))


def delete(request, bid):
    BookInfo.objects.get(pk=bid).delete()
    # bs = BookInfo.objects.all()
    bs = get_list_or_404(BookInfo)
    # temp = loader.get_template('booktest/list.html')
    # con = {'booklist': bs}
    # result = temp.render(con)
    # return render(request, 'booktest/list.html', context=con)
...
def addherohandle(request, bookid):
    ...
    # return HttpResponseRedirect(reverse('booktest:bookdetail', args=[book.id]), {'book': book})
    return redirect(reverse('booktest:bookdetail', args=[book.id]), {'book': book})
...
def edithandle(request, bid):
    ...
    book.save()
    # return HttpResponseRedirect(reverse('booktest:booklist'))
    return redirect(reverse('booktest:booklist'))
...
def deletehero(request, hid):
    ...
    hero.delete()
    # return HttpResponseRedirect('/booktest/detail/'+str(book.id)+'/', {'book': book})
    # return HttpResponseRedirect(reverse('booktest:bookdetail', book.id), {'book': book})
    return redirect(reverse('booktest:bookdetail', book.id), {'book': book})
